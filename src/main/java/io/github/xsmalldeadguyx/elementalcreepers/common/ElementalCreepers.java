...
public class ElementalCreepers {
	...
	public static List<RegistryObject<SpawnEggItem>> EGG_ITEMS = new ArrayList<RegistryObject<SpawnEggItem>>();
	...
	public static RegistryObject<EntityType<FriendlyCreeper>> FRIENDLY_CREEPER = ENTITIES.register("friendly_creeper",
			() -> EntityType.Builder.of(FriendlyCreeper::new, MobCategory.CREATURE).sized(0.6F, 1.7F)
					.clientTrackingRange(10).build(ResourceKey.create(Registries.ENTITY_TYPE,
							ResourceLocation.fromNamespaceAndPath(MODID, "friendly_creeper"))));
	...
	public ElementalCreepers(FMLJavaModLoadingContext context) {
		...
		EGG_ITEMS.add(ITEMS.register(
				"friendly_creeper_spawn_egg", () -> new SpawnEggItem(FRIENDLY_CREEPER.get(), new Item.Properties().setId(ITEMS.key("friendly_creeper_spawn_egg")))));
		...
		CREATIVE_MODE_TABS.register("elemental_creeper_egg_tab",
				() -> CreativeModeTab.builder().withTabsBefore(CreativeModeTabs.COMBAT)
						...
						.icon(() -> EGG_ITEMS.get(3).get().getDefaultInstance()).displayItems((parameters, output) -> {
							for (RegistryObject<SpawnEggItem> egg : EGG_ITEMS) {
	...
	public static <T extends ElementalCreeper> RegistryObject<EntityType<T>> registerCreeper(
			EntityFactory<T> creeperFactory, String name, @Nullable Color colour, @Nullable Color colour2) {
		ResourceKey<EntityType<?>> key =  ResourceKey.create(Registries.ENTITY_TYPE, ResourceLocation.fromNamespaceAndPath(MODID, name));
		RegistryObject<EntityType<T>> entityRO = ENTITIES.register(name, () -> EntityType.Builder
				.of(creeperFactory, MobCategory.MONSTER).sized(0.6F, 1.7F).clientTrackingRange(8).build(key));

		if (colour != null) {
			RegistryObject<SpawnEggItem> eggRO = ITEMS.register(name + "_spawn_egg",
					() -> new SpawnEggItem(entityRO.get(), new Item.Properties().setId(ITEMS.key(name + "_spawn_egg"))));
			EGG_ITEMS.add(eggRO);
		}
		return entityRO;
	}
	...
	public void spawnPlacementRegisterEvent(SpawnPlacementRegisterEvent event) {
		event.register(CELEBRATION_CREEPER.get(), SpawnPlacementTypes.ON_GROUND,
				Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(COOKIE_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(DARK_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(EARTH_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(ELECTRIC_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(FAKE_ILLUSION_CREEPER.get(), SpawnPlacementTypes.ON_GROUND,
				Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(FIRE_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(ICE_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(ILLUSION_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(GHOST_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(LIGHT_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(MAGMA_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(PSYCHIC_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(REVERSE_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(SPIDER_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);
		event.register(WATER_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Monster::checkMonsterSpawnRules, Operation.AND);

		event.register(FRIENDLY_CREEPER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				Animal::checkAnimalSpawnRules, Operation.AND);
	}
}
